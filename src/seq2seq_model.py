  ...
  def __init__(self,
    ...
    if architecture == "basic":
      ...
      with vs.variable_scope("basic_rnn_seq2seq"):
        ...
        n = tf.Variable(tf.zeros([1,batch_size, self.input_size], dtype=tf.float32), trainable=True, dtype=tf.float32)




        #NN = tf.concat([n,N],axis = 0)
        #NN = tf.

        #dec_in = (dec_in+NN)/2.0
        #m = tf.Variable(tf.zeros([1,1024],dtype=tf.float32),trainable=True,dtype =tf.float32 )
        #enc_state = tf.concat([m,enc_state],axis=0)


        #second AttentionType:Gloabl
        encoder_outputs, enc_state = tf.nn.dynamic_rnn(cell, enc_in, dtype=tf.float32, time_major=True)  # Encoder

#local
        U = tf.tanh(tf.einsum('ijk,kl->ijl', encoder_outputs, attention_W1))
        a = tf.nn.softmax(U,2)
        s = encoder_outputs+a
        # s = tf.concat([n,s],0)
        # dec_in = (s + dec_in)
