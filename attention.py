...
from util_Attention import softmax, orthogonal_initializer
...
def inference(documents, doc_mask, query, query_mask):
    ...
    y_hat = tf.pack(
        [tf.unsorted_segment_sum(attentions, sentence_ids, FLAGS.vocab_size) for (attentions, sentence_ids) in
         unpacked_s])
