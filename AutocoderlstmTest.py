...
x = tf.placeholder("float32", [None, n_sequence, n_input])
...
with tf.variable_scope("input_full"):
    wb11={
        'weights': tf.Variable(tf.random_normal([n_input, n_hidden_1], seed=1,dtype=tf.float32),dtype=tf.float32),
        'biases': tf.Variable(tf.constant(0.1, shape=[n_hidden_1, ],dtype=tf.float32),dtype=tf.float32),
    }
    wb12 = {
        'weights': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_1], seed=1,dtype=tf.float32),dtype=tf.float32),
        'biases': tf.Variable(tf.constant(0.1, shape=[n_hidden_1, ],dtype=tf.float32),dtype=tf.float32),
    }
    wb13 = {
        'weights': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_1], seed=1,dtype=tf.float32),dtype=tf.float32),
...
with tf.variable_scope("out_full"):
    ...
    wb9 = {
        'weights': tf.Variable(tf.random_normal([n_hidden_1, n_input], seed=1,dtype=tf.float32),dtype=tf.float32),
                ...
                for epoc in range(training_epochs):
                    ...
                    if epoc % 2 == 0:
                        if not os.path.exists(ckpt_dir):
                            ...
                            saver.save(sess, ckpt_dir + "/model.ckpt", global_step=epoc+1)
                            global_step.assign(epoc).eval()
                        else:
                            if l and l<remember:
                                saver.save(sess, ckpt_dir + "/model.ckpt", global_step=epoc + 1)
                                global_step.assign(epoc).eval()
